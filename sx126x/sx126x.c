...
}RadioRegisters_t;//寄存器结构体

/*!
 * \brief Holds the internal operating mode of the radio
 */
static RadioOperatingModes_t OperatingMode;//操作模式，lora和fsk
 ...
 */
volatile uint32_t FrequencyError = 0;//射频错误信息

/*!
 * \brief Hold the status of the Image calibration
 */
static bool ImageCalibrated = false;//镜像校准？
 ...
 */
void SX126xOnDioIrq( void );//DIO中断

/*!
 * \brief 设置沦陷模式，干嘛的？
 */
void SX126xSetPollingMode( void ); //DIO
...
/*
 * \brief Process the IRQ if handled by the driver，处理中断
 */
...
void SX126xInit( DioIrqHandler dioIrq )
{
    ...
    SX126xIoIrqInit( dioIrq );//dio中断初始化

    SX126xWakeup( );//唤醒
    SX126xSetStandby( STDBY_RC );//待机

#ifdef USE_TCXO//如果使用温补晶振
{
    CalibrationParams_t calibParam;//校准参数

    //设置DIO3作为温补晶振控制
    SX126xSetDio3AsTcxoCtrl( TCXO_CTRL_1_8V, SX126xGetBoardTcxoWakeupTime( ) << 6 ); // convert from ms to SX126x time base
    calibParam.Value = 0x7F;//校准参数值
    SX126xCalibrate( calibParam );//校准
}
#endif

    SX126xSetDio2AsRfSwitchCtrl( true );//设置DIO2作为射频开关控制
    OperatingMode = MODE_STDBY_RC;//待机模式
}
...
{
    OperatingMode = mode;
#if defined( USE_RADIO_DEBUG )//射频调试模式？
    switch( mode )
    {
        case MODE_TX:
            SX126xDbgPinTxWrite( 1 );//debugpin?用于led指示的，没找到指示，可以用于以后做led指示
...
void SX126xSetCrcSeed( uint16_t seed )
{
    ...
    switch( SX126xGetPacketType( ) )//获取包类型
    {
...
{
    SX126xAntSwOff( );//ant关闭

    SX126xWriteCommand( RADIO_SET_SLEEP, &sleepConfig.Value, 1 );//设置休眠
...
{
    uint8_t en = enable;
    SX126xWriteCommand( RADIO_SET_STOPRXTIMERONPREAMBLE, &en, 1 );
}
...
void SX126xSetModulationParams( ModulationParams_t *modulationParams )
{
    ...
    case PACKET_TYPE_LORA:
        n = 4;
        buf[0] = modulationParams->Params.LoRa.SpreadingFactor;//扩频因子
        buf[1] = modulationParams->Params.LoRa.Bandwidth;//带宽
        buf[2] = modulationParams->Params.LoRa.CodingRate;//码率
        buf[3] = modulationParams->Params.LoRa.LowDatarateOptimize;//低速率优化
...
void SX126xSetPacketParams( PacketParams_t *packetParams )
{
    ...
    case PACKET_TYPE_LORA:
        ...
        buf[1] = packetParams->Params.LoRa.PreambleLength;//前导码长度
        buf[2] = packetParams->Params.LoRa.HeaderType;//头部类型
        buf[3] = packetParams->Params.LoRa.PayloadLength;//负载类型
        buf[4] = packetParams->Params.LoRa.CrcMode;//crc模式
        buf[5] = packetParams->Params.LoRa.InvertIQ;//翻转IQ情况
        break;
    default:
    case PACKET_TYPE_NONE:
        return;
